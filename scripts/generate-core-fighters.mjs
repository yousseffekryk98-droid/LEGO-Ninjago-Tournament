...
async function generate(id,primary,accent,skin){
  ...
  const output=resolve(`public/assets/models/fighters/${id}.glb`);await mkdir(dirname(output),{recursive:true});await writeFile(output,new Uint8Array(out));console.log(`Generated ${output} (${total} bytes)`);
}
