...
export const POWERUPS: Array<{ id: PowerupId; name: string; description: string; cost: number; icon: string }> = [
  ...
  { id: 'charged-scroll', name: 'Charged Scroll', description: 'Start the next run with 55% special meter.', cost: 1600, icon: '↻' },
  { id: 'battle-focus', name: 'Battle Focus', description: '+18% damage and +8% speed for the next run.', cost: 2400, icon: '⚔' }
];
...
function readMainSave() {
  try {
    return JSON.parse(localStorage.getItem(SAVE_KEY) ?? '{}') as { bankStuds?: number } & Record<string, unknown>;
  } catch {
    return { bankStuds: 0 } as { bankStuds?: number } & Record<string, unknown>;
  }
}
